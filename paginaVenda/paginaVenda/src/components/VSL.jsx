        ...
        >
          ...
          {!showButtons && (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center mt-6 text-sm md:text-base text-yellow-500 font-semibold"
            >
              ⏱️ Assista o vídeo para desbloquear os botões
            </motion.p>
          )}
          ...
          >
            <motion.button
              ...
              whileHover={showButtons ? { scale: 1.05 } : {}}
              whileTap={showButtons ? { scale: 0.95 } : {}}
              disabled={!showButtons}
              className={`px-12 py-4 text-lg font-bold rounded-lg shadow-2xl w-full sm:w-auto transition-all ${
                showButtons
            ...
            <motion.a
              href={showButtons ? "https://wa.me/5543999748808?text=Olá!%20Gostaria%20de%20saber%20mais%20sobre%20o%20CineStream" : "#"}
              target={showButtons ? "_blank" : "_self"}
              rel="noopener noreferrer"
              onClick={(e) => !showButtons && e.preventDefault()}
              whileHover={showButtons ? { scale: 1.05 } : {}}
              whileTap={showButtons ? { scale: 0.95 } : {}}
              className={`px-12 py-4 text-lg font-bold rounded-lg shadow-2xl flex items-center gap-2 w-full sm:w-auto justify-center transition-all ${
                showButtons
